...
- **The settings tab's single source of truth is `getSettingDefinitions()`** (Obsidian 1.13 declarative API); `display()` is only the < 1.13 fallback interpreter over the same definitions. The 1.13 renderer **snapshots the definitions at `addSettingTab()` time and never calls `display()`** — anything that changes data shown in settings (sync completion, sign-out, handle capture) must call `plugin.refreshSettingsTab()` or the tab silently shows stale values. Guard 1.13-only API *calls* with `requireApiVersion('1.13.0')` (in the `if` test, or the left side of `&&`) — `typeof` guards fail the review bot's `no-unsupported-api` rule. When minAppVersion is someday raised to ≥ 1.13: delete `display()`/the `refresh()` fallback and swap `setWarning` → `setDestructive` in the same commit.
- **Every plugin webview must pass `useragent: X_WEBVIEW_UA`** (types.ts) alongside the session partition. X's login flow (the "Confirm your account" bot check, issue #8) silently rejects clients whose UA advertises Electron/Obsidian — buttons click but do nothing. The UA is a frozen stock-Chrome string built from `Platform` (OS token) + `process.versions.chrome` — never `navigator.*`, which the review bot rejects (Error-level, "use the Platform API"). A new webview creation site without it would split the shared session's identity and re-trip the login gate.
- **Signed-in account identity comes from `x.com/i/api/1.1/account/multi/list.json`** (the account switcher's own endpoint: `users[].screen_name`, `is_active` marks the current account when several are signed in) — `1.1/account/settings.json` was removed by X (404 as of 2026-08-18). Captured best-effort once per sync into `settings.signedInHandle` (`captureSignedInHandle`, view.ts); never probed live from the settings tab. This endpoint is also the natural identity source for multi-account support (issue #6).
...
When replying to a GitHub issue and waiting on the reporter, add the `awaiting-response` label — `.github/workflows/stale.yml` warns at 14 quiet days and auto-closes 7 days later, but only for issues carrying that label. Reporter activity clears it automatically.
