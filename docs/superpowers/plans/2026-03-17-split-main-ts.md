...
**Goal:** Split the 1100-line `obsidian-plugin/main.ts` into four focused modules with no behaviour changes.

**Architecture:** Extract each class into its own file. A shared `types.ts` holds the data interface and the `VIEW_TYPE` constant so neither `view.ts` nor `main.ts` need to import from each other just for that constant. Circular imports (`modal`/`view`/`settings-tab` → `main` → `modal`/`view`/`settings-tab`) are safe because no values are consumed at module-evaluation time — only inside class methods that run after all modules are loaded.

**Tech Stack:** TypeScript, Obsidian Plugin API, esbuild (bundler — handles circular imports fine)

---

## Chunk 1: Create shared types and new modules

### Task 1: Create `types.ts`

**Files:**
- Create: `obsidian-plugin/types.ts`

This file holds the two items shared across all modules so there is no circular import just for constants/interfaces.

- [ ] **Step 1: Create the file**

```typescript
// obsidian-plugin/types.ts
...
- [ ] **Step 2: Verify build still passes (main.ts unchanged yet — this is additive)**

```bash
cd /Users/fei.hu/Fei.Codes/obsidian-plugins/x-bookmarks-sync
...
```bash
git add obsidian-plugin/types.ts
git commit -m "refactor: add types.ts with VIEW_TYPE and XBookmarksSyncData"
```

---

### Task 2: Create `modal.ts`

**Files:**
- Create: `obsidian-plugin/modal.ts`
- Source: `obsidian-plugin/main.ts` lines 6–118 (`BookmarkSelectionModal`)

- [ ] **Step 1: Create the file**

Copy the `BookmarkSelectionModal` class verbatim from `main.ts` lines 6–118, adding the imports it needs:

```typescript
// obsidian-plugin/modal.ts
...
Note: `import type XBookmarksSync from './main'` uses a type-only import. The `plugin` property is only used through its methods at runtime (not as a constructor call), so the type-only import is sufficient and avoids a live circular reference.

- [ ] **Step 2: Verify build (main.ts still has the original class — no conflict yet)**
...
```bash
git add obsidian-plugin/modal.ts
git commit -m "refactor: extract BookmarkSelectionModal to modal.ts"
```

---

### Task 3: Create `settings-tab.ts`

**Files:**
- Create: `obsidian-plugin/settings-tab.ts`
- Source: `obsidian-plugin/main.ts` lines 782–930 (`XBookmarksSyncSettingTab`)

- [ ] **Step 1: Create the file**

```typescript
// obsidian-plugin/settings-tab.ts
...
Note: `Notice` is intentionally excluded — it is not used in this class.
...
Expected: clean build. If there is an "unused import" warning for `Notice`, remove it from the import line.

- [ ] **Step 3: Commit**

```bash
git add obsidian-plugin/settings-tab.ts
git commit -m "refactor: extract XBookmarksSyncSettingTab to settings-tab.ts"
```

---

### Task 4: Create `view.ts`

**Files:**
- Create: `obsidian-plugin/view.ts`
- Source: `obsidian-plugin/main.ts` lines 120–780 (`XBookmarksView`)

- [ ] **Step 1: Create the file**

```typescript
// obsidian-plugin/view.ts
...
- [ ] **Step 2: Verify build (all new files exist, main.ts still has originals)**
...
```bash
git add obsidian-plugin/view.ts
git commit -m "refactor: extract XBookmarksView to view.ts"
```

---

## Chunk 2: Trim main.ts and final verification

### Task 5: Rewrite `main.ts` to import from new modules

**Files:**
- Modify: `obsidian-plugin/main.ts` (replace entire contents)

Now that all three classes exist in their own files, replace `main.ts` with just the plugin class and its imports. The entire file becomes:

- [ ] **Step 1: Replace `main.ts` with the trimmed version**
...
- [ ] **Step 2: Build and verify — this is the critical check**

```bash
npm run build:plugin
```

Expected: clean build, no TypeScript errors. The bundled `main.js` should be approximately the same size as before (esbuild bundles all modules into one file).

If there are errors:
- `Cannot find module './view'` → check that `view.ts` exists at `obsidian-plugin/view.ts`
- `Type 'XBookmarksSync' is not assignable` from circular import → change `import type` to a regular `import` in the affected file
- Unused import warnings → remove the unused name from the import line
...
Open Obsidian, reload the plugin (Settings → Community plugins → disable/enable, or use the hotkey), and verify:
- The X Bookmarks ribbon icon opens the webview ✓
- Plugin settings tab opens and shows all three settings ✓
- No console errors in Obsidian DevTools ✓

- [ ] **Step 4: Commit**

```bash
git add obsidian-plugin/main.ts
git commit -m "refactor: trim main.ts to plugin class only; wire imports from new modules"
```
